    ...
    if((check(s1) == 0 && check(s2) == 1)){
        if(check(res) == 1){
            for(int i = mx_pow_rec(10, mx_strlen(res) - 1) - 1; i < (mx_pow_rec(10, mx_strlen(res))); i++){
                if((i - mx_atoi(s1)) > 0 && digit_count(i - mx_atoi(s1)) == mx_strlen(s2)){
            ...
            if(mx_atoi(res) > mx_atoi(s1))
                print_l(mx_atoi(s1), '+',mx_atoi(res) - mx_atoi(s1), mx_atoi(res));
        }
    }
    else if ((check(s1) == 1 && check(s2) == 0)){
        if(check(res) == 1){
            for(int i = mx_pow_rec(10, mx_strlen(res) - 1) - 1; i < (mx_pow_rec(10, mx_strlen(res))); i++){
                if((i - mx_atoi(s2)) > 0 && digit_count(i - mx_atoi(s2)) == mx_strlen(s1)){
        ...
        if(check(res) == 1){
            for (int i = mx_pow_rec(10, mx_strlen(s1) - 1) - 1; i < (mx_pow_rec(10, mx_strlen(s1))); i++) {
                for (int j = mx_pow_rec(10, mx_strlen(s2) - 1) - 1; j < (mx_pow_rec(10, mx_strlen(s2))); j++) {
                    if(digit_count(i + j) == mx_strlen(res)){
                        print_l(i, '+', j, i + j);
                        }
                    }
                }
