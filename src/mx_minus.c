    ...
    if((check(s1) == 0 && check(s2) == 1)){
        if(check(res) == 1){
            for(int i = mx_pow_rec(10, mx_strlen(res) - 1) - 1; i < (mx_pow_rec(10, mx_strlen(res))); i++){
                if((mx_atoi(s1) - i) > 0 && digit_count(mx_atoi(s1) - i) == mx_strlen(s2)){
                    print_l(mx_atoi(s1), '-', mx_atoi(s1) - i, i);
                }
            }
        }
        else if(check(res) == 0) {
            if(mx_atoi(s1) > mx_atoi(res))
        ...
        if(check(res) == 1){
            for(int i = mx_pow_rec(10, mx_strlen(res) - 1) - 1; i < (mx_pow_rec(10, mx_strlen(res))); i++){
                if(digit_count(i + mx_atoi(s2)) == mx_strlen(s1)){
        ...
        for (int i = mx_pow_rec(10, mx_strlen(s1) - 1); i < (mx_pow_rec(10, mx_strlen(s1))); i++) {
            for (int j = mx_pow_rec(10, mx_strlen(s2) - 1); j < (mx_pow_rec(10, mx_strlen(s2))); j++) {
                if(i - j == mx_atoi(res) && digit_count(i) == mx_strlen(s1) && digit_count(j) == mx_strlen(s2) * i > j)
                    print_l(i, '-', j, mx_atoi(res));
            }
        }
    }
    else if ((check(s1) == 0 && check(s2) == 0) && check(res) == 1 && mx_atoi(s1) >= mx_atoi(s2)){
